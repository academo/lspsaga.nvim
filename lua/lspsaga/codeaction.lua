...
local get_current_winid = function() return api.nvim_get_current_win() end
...
if vim.tbl_isempty(vim.fn.sign_getdefined(SIGN_NAME)) then
    vim.fn.sign_define(SIGN_NAME, {
        text = config.code_action_icon,
        texthl = "LspSagaLightBulbSign"
    })
end
...
local function _update_sign(line)
    ...
    if Action[winid] and Action[winid].lightbulb_line ~= 0 then
        vim.fn.sign_unplace(SIGN_GROUP,
                            {id = Action[winid].lightbulb_line, buffer = "%"})
    end

    if line then
        vim.fn.sign_place(line, SIGN_GROUP, SIGN_NAME, "%", {
            lnum = line + 1,
            priority = config.code_action_prompt.sign_priority
        })
        if (Action[winid] ~= nil) then
...
function Action:render_action_virtual_text(line, diagnostics)
    return function(_, _, actions)
        if actions == nil or type(actions) ~= "table" or
            vim.tbl_isempty(actions) then
            if config.code_action_prompt.virtual_text then
                _update_virtual_text(nil)
            end
            if config.code_action_prompt.sign then _update_sign(nil) end
        else
    ...
    return function(_, _, response)
        ...
        local from_other_servers = function()
            ...
            for _, action in pairs(response) do
                ...
                local action_title = '[' .. #self.actions .. ']' .. ' ' ..
                                         action.title
        ...
        if self.actions and next(self.actions) ~= nil then
            ...
            for i, _ in pairs(other_actions) do
                vim.fn.matchadd('LspSagaCodeActionContent',
                                '\\%' .. #self.actions + 1 + i .. 'l')
            end
        else
        ...
        self.action_bufnr, self.action_winid =
            window.create_win_with_border(content_opts)
        api.nvim_command(
            'autocmd CursorMoved <buffer> lua require("lspsaga.codeaction").set_cursor()')
        api.nvim_command(
            "autocmd QuitPre <buffer> lua require('lspsaga.codeaction').quit_action_window()")

        api.nvim_buf_add_highlight(self.action_bufnr, -1,
                                   "LspSagaCodeActionTitle", 0, 0, -1)
        api.nvim_buf_add_highlight(self.action_bufnr, -1,
                                   "LspSagaCodeActionTruncateLine", 1, 0, -1)
        for i = 1, #contents - 2, 1 do
            api.nvim_buf_add_highlight(self.action_bufnr, -1,
                                       "LspSagaCodeActionContent", 1 + i, 0, -1)
        end
...
function Action:apply_action_keys()
    ...
    for func, keys in pairs(actions) do apply_keys(func, keys) end
end

local action_call_back = function(_, _) return Action:action_callback() end
...
lspaction.do_code_action = function() Action:do_code_action() end

lspaction.quit_action_window = function() Action:quit_action_window() end

lspaction.range_code_action = function() Action:range_code_action() end

lspaction.set_cursor = function() Action:set_cursor() end
